...
void ControllersHandler::remove_controller(SDL_JoystickID id) {
  std::vector<GamepadController *>::iterator it =
      std::find_if(this->controllers.begin(), this->controllers.end(),
                   [id](GamepadController *cont) { return cont->get_gamepad_id() == id; });
